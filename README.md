...
- タイムライン右側のメタデータ欄でタイトル・前提条件（必殺ゲージMAX）・説明を設定
  （プレビューでは読み取り表示、エクスポート/共有にも含まれる）
- タブで複数のスキル回しを同時に管理（編成＝PT も行動もタブごとに独立）
- タブ単位で名前を付けてブラウザに保存／読込（読込は新しいタブとして開く）。
  全タブの自動保存（ページ読み込み時に復元）とは別枠の localStorage キーに保存する
- 「セッション（全タブ）」を手動で保存／復元（自動保存とは別枠のスナップショット。
  復元は全タブを上書き、既存タブがあれば確認ダイアログ）
- 「全タブをURLで共有」で全タブをまとめた共有リンク（`#w=...`）を生成。開くと全タブを上書き復元
  （既存のタブがある場合は上書き確認ダイアログを表示）
